...
class DataManager: ObservableObject {
    ...
    private var calculationService: CardCalculationService
    ...
    private init() {
        ...
        self.calculationService = CardCalculationService()
    ...
    // MARK: - Card Calculation Methods
    
    func getBirthCard() -> Card {
        guard let birthDate = userProfile.birthDate else {
            return getCard(by: 1)
        }
        ...
        let cardId = calculationService.bc(month: month, day: day)
        return getCard(by: cardId)
    }
    
    func getDailyCard(for date: Date? = nil) -> DailyCardResult {
        guard let birthDate = userProfile.birthDate else {
            return DailyCardResult(cardId: 1, planet: "Mercury", planetNum: 1)
        }
        
        let targetDate = date ?? explorationDate ?? Date()
        let birthCard = getBirthCard()
        
        let result = calculationService.dc(birthDate: birthDate, birthCard: birthCard.id, targetDate: targetDate)
        
        // Convert cardId to Card object for the result
        let card = getCard(by: result.cardId)
        return DailyCardResult(card: card, planet: result.planet, planetNum: result.planetNum)
    }
    
    func getYearlyCard() -> Card {
        guard let birthDate = userProfile.birthDate else {
            return getCard(by: 1)
        }
        
        let birthCard = getBirthCard()
        let age = calculationService.chron(birthDate: birthDate)
        let cardId = calculationService.yc(birthCard: birthCard.id, age: age)
        
        return getCard(by: cardId)
    }
    
    func get52DayCard() -> Card {
        guard let birthDate = userProfile.birthDate else {
            return getCard(by: 1)
        }
        ...
        let birthCard = getBirthCard()
        let age = calculationService.chron(birthDate: birthDate)
        let period = calculationService.p(birthDate: birthDate, targetDate: explorationDate ?? Date())
        let cardId = calculationService.fc(birthCard: birthCard.id, age: age, period: period)
    ...
    func getCurrentAge() -> Int {
        ...
        return calculationService.chron(birthDate: birthDate)
    }
    ...
    func getCurrentPeriod() -> Int {
        ...
        return calculationService.p(birthDate: birthDate, targetDate: explorationDate ?? Date())
    }
    ...
    func getTodayCard() -> Card {
        let dailyResult = getDailyCard()
        return dailyResult.card ?? getCard(by: dailyResult.cardId)
    }
