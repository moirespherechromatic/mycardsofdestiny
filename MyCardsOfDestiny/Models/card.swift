...
// MARK: - Karma Connection Model
struct KarmaConnection: Codable {
    let cards: [Int]
    let description: String
}

// MARK: - Daily Card Result
struct DailyCardResult {
    ...
    // Initializer with Card object (for DataManager)
    init(card: Card, planet: String, planetNum: Int) {
        self.card = card
        self.cardId = card.id
        self.planet = planet
    ...
    // Initializer with cardId (for CardCalculationService to avoid circular dependency)
    init(cardId: Int, planet: String, planetNum: Int) {
        self.card = nil
        self.cardId = cardId
        self.planet = planet
        self.planetNum = planetNum
...
struct CardConstants {
    // Card IDs matching JavaScript constants
