...
struct BirthCardView: View {
    ...
    // Dismiss card detail view with animation - UPDATED TO MATCH 52-DAY VIEW
    ...
    // UPDATED TO MATCH 52-DAY VIEW ANIMATION TIMING
        ...
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    // Birth Card Image - Tappable
                    Button(action: showBirthCard) {
                        if let uiImage = loadCardImage(for: birthCard) {
    ...
    private var karmaConnectionsSection: some View {
        VStack(spacing: 24) {
            ...
            // Show first karma connection cards in a horizontal layout
                ...
                HStack(spacing: 40) {
                    ForEach(cards.prefix(2), id: \.id) { card in
                        Button(action: {
                            ...
                            if let uiImage = loadCardImage(for: card) {
                                Image(uiImage: uiImage)
                                    .resizable()
                                    .aspectRatio(contentMode: .fit)
