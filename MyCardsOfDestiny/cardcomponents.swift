...
struct TappableCard: View {
    ...
    // Convenience initializer for compatibility with updated view files
    init(card: Card, onTap: @escaping () -> Void) {
        self.card = card
        self.size = AppConstants.CardSizes.medium
        self.cornerRadius = AppConstants.CornerRadius.cardLarge
        self.action = onTap
    }
